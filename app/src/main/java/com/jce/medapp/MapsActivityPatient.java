...
public class MapsActivityPatient extends FragmentActivity implements OnMapReadyCallback {

    private GoogleMap mMap;
    LatLng patientLocation, currentLocation;
    Marker source, destination;
    LocationManager mLocationManager;
    boolean flag = true;
    Bundle bundle;
    Polyline polyline;
    String driver,patient;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_maps_patient);
    ...
    public void addStaticMarker()
    {
        ParseQuery<ParseObject> query = ParseQuery.getQuery("UserLocation");
        query.whereEqualTo("Username",patient);
        try{
            List<ParseObject> value = query.find();
            ParseObject val= value.get(0);
            Log.e("pp",val.getParseGeoPoint("Location")+"");
            String did = val.getObjectId();
            patientLocation = new LatLng(val.getParseGeoPoint("Location").getLatitude(),val.getParseGeoPoint("Location").getLongitude());
            Log.e("Message",patientLocation.toString());
            if (destination!=null)
            {
                destination.remove();
                destination = mMap.addMarker(new MarkerOptions().position(patientLocation).title("Patient").icon(BitmapDescriptorFactory.fromResource(R.drawable.patient)));
    ...
    LocationListener locationListener = new LocationListener() {
        ...
        public void onLocationChanged(Location location) {
            ...
            if (source!=null)
                source.remove();
            source = mMap.addMarker(new MarkerOptions().title("ME").position(currentLocation).icon(BitmapDescriptorFactory.fromResource(R.drawable.ambulance)));

            getDirections(currentLocation,patientLocation);
            pushCurrentLocation(currentLocation,driver);
//            LatLng center = new LatLng((currentLocation.latitude+patientLocation.latitude)/2,(currentLocation.longitude+patientLocation.longitude)/2);
//            mMap.animateCamera(CameraUpdateFactory.newLatLngZoom(center, (float) 13.5));
            LatLngBounds.Builder builder = new LatLngBounds.Builder();
            builder.include(currentLocation);
            builder.include(patientLocation);
